...
//        CGFloat dy = 0.3f;
        ...
        if ( i < 7 ) {
            one.center = CGPointMake(oneWidth * (j + 1) - FIRST_MASU_POSI_X,
                                     SCREEN_SIZE.width * FIRST_MASU_POSI_Y);
            one.alpha = 0.1 * (j + 1);
            
        } else if ( i >= 7 && i < 14 ) {
            j = i - 7;
            one.center = CGPointMake(oneWidth * (j + 1) - FIRST_MASU_POSI_X,
                                     SCREEN_SIZE.width * (FIRST_MASU_POSI_Y + MASU_dy_FOR_Twentyone));
            one.alpha = 0.85 - (0.1 * (j + 1));
            
        } else {
            j = i - 14;
            one.center = CGPointMake(oneWidth * (j + 1) - FIRST_MASU_POSI_X,
                                     SCREEN_SIZE.width * (FIRST_MASU_POSI_Y + MASU_dy_FOR_Twentyone*2));
            one.alpha = 0.1 * (j + 1);
